...
L'equazione
$$
f(x,y) = f(x_0) \text{ con } x_0 \in \Omega
$$
ha altre soluzioni oltre a $x_0$. La condizione di $\partial_y f(x_0) \neq 0$ ci consente di effettuare un cambiamento delle coordinate del nostro sistema e, dunque, scegliere
$$
(x, y) \mapsto (x, f(x,y))
$$
e poniamo $\tilde{f}(x, y) = (x, f(x, y))$. \\
Le soluzioni che soddisfano questa equazione in questo nuovo sistema di coordinate saranno una retta e possiamo andare ad invertire la nostra funzione localmente, infatti considerando
$$
\tilde{f}(x_0, y_0) = (x_0, f(x_0, y_0)) \implies D\tilde{f}(x_0, y_0) = \begin{pmatrix}
1 & 0 \\
\partial_x f(x_0, y_0) & \partial_y f(x_0, y_0)
\end{pmatrix}
$$
avremo che il determinante di questa funzione è pari a $\partial_y f(x_0, y_0)$ che, per ipotesi, è diverso da zero e questo ci consente di usare il teorema della funzione inversa a questa funzione. Conseguentemente, esisterà una funzione $\tilde{g}$ tale che $\tilde{f}(\tilde{g}(x, y)) = (x, y) = \tilde{f}(g_1(x, y), g_2(x,y)) = (g_1(x, y), f(g_1(x, y), g_2(x, y)) \implies$
$$
\implies \begin{cases}
g_1(x, y) = x \\
...
\end{cases}
$$
dunque ponendo $g(x) = g_2(x, f(x_0, y_0))$ otteniamo che
$$
f(x, g(x)) = f(x_0, y_0)
$$
Per dimostrare la formula della derivata basta derivare quest'ultima relazione:
$$
\partial_x f(x, g(x)) + \partial_y f(x, g(x)) \partial_x g(x) = 0 \implies \partial_x g(x) = - \frac{\partial_x f(x, g(x))}{\partial_y f(x, g(x))}
$$
\end{proof}
\begin{remark}
Possiamo riformulare il teorema della funzione implicita nei punti in cui $\partial_y f(x_0) = 0$ ma $\partial_x f(x_0)$: infatti, la versione più generale del teorema richiede esclusivamente che il gradiente della funzione $f$ sia diverso da zero, in maniera tale da "riorganizzare" il determinante di $\tilde{f}$ nella dimostrazione sopra per non farlo diventare $0$.
\end{remark}
