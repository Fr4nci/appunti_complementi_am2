...
\end{exercise}
\begin{proof} \hspace{1cm} \\
	Il fatto che $\bar{A} \subseteq A \cup \text{Fr} \, A$ si ottiene osservando che. Il viceversa si ottiene dal seguente ragionamento: supponiamo che $\exists x \in A \cup \text{Fr} \, A$ tale che $x \not\in \bar{A}$, allora escludiamo che $x \not\in A$ siccome $A \subseteq \bar{A}$. Dunque $x \in \text{Fr} \, A$ ma $\forall x \in \text{Fr} \, A$ abbiamo che
	$\forall r > 0, B(x, r) \cap A \neq \emptyset \wedge B(x, r) \cap A^c \neq \emptyset$, dunque $x$ è un punto di aderenza di $A$ il che è un assurdo siccome implicherebbe che $x \in \bar{A}$.
\end{proof}
Strutturalmente parlando, nella dimostrazione qua sotto si deve usare implicitamente il fatto che $\bar{A} = A \cap \text{Fr} \, A$, dunque la propongo sotto la precedente proposizione, tuttavia nel corso viene proposto di risolverlo senza utilizzare questa sopra.
\begin{proof} \hspace{1cm} \\
