...
\begin{definition}[misura di Hausdroff $2-$dimensionale]
	...
	$$
		\mathcal{H}^2(A) = \sup_{\delta > 0} \inf \{ \sum_{j=0}^{+\infty} \frac{\pi}{4} (\diam E_j)^2 | A \subseteq \bigcup_{j=0}^{+\infty} E_j \}
	$$
	per tutti i possibili ricoprimenti $\{ E_i \}_i$ per cui $\sup_{i \geq 0} \diam E_i < \delta$, dove $\diam E = \sup\limits_{x, y \in E} |x-y|$
\end{definition}
...
pertanto
$$
\sigma(\Sigma) = \int_{(a, b) \times (0, 2\pi)} = \int_{0}^{2\pi} \int_a^b \gamma_1(t) |\dot{\gamma}(t)|dt d\varphi = 2 \pi \int_a^b \gamma_1(t) |\dot{\gamma}(t)|dt = 2 \pi L \int_\gamma x dl
$$
